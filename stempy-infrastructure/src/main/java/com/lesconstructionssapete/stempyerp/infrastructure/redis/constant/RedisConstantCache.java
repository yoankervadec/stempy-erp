package com.lesconstructionssapete.stempyerp.infrastructure.redis.constant;
...
public class RedisConstantCache implements ConstantCache {
  ...
  private final CacheProvider redis;
  private final ConstantService service;

  public RedisConstantCache(CacheProvider redis, ConstantService service) {
    this.redis = redis;
  ...
  private <T> List<T> getOrLoad(
    ...
    try {
      String cached = redis.getRaw(key);
